...
const decision={action:'approve',reason:'Checked synthetic facts',text:'Reviewed explanation'}
test('starts at human review with checkpoint history',async()=>{
 const s=createReviews(),r=await s.start(claims[0].id,'start')
...
test('approval releases edited text without changing claim',async()=>{
 const s=createReviews(),before=JSON.stringify(claims),r=await s.start(claims[0].id,'start')
 const done=await s.review(r.id,decision,'review')
 assert.equal(done.status,'approved');assert.equal(done.answer,decision.text)
...
test('rejection does not release the draft',async()=>{
 ...
 assert.equal(done.status,'rejected');assert.match(done.answer,/No response released/)
})
test('duplicate starts share a single thread',async()=>{
 const s=createReviews(),[a,b]=await Promise.all([s.start(claims[0].id,'same'),s.start(claims[0].id,'same')])
...
test('concurrent opposing reviews permit only one decision',async()=>{
 const s=createReviews(),r=await s.start(claims[0].id,'start')
 const results=await Promise.allSettled([s.review(r.id,decision,'a'),s.review(r.id,{...decision,action:'reject'},'b')])
 assert.equal(results.filter(r=>r.status==='fulfilled').length,1)
})
test('completed request replay does not create checkpoints',async()=>{
 const s=createReviews(),r=await s.start(claims[0].id,'start')
 const done=await s.review(r.id,decision,'a'),n=(await s.history(r.id)).length
...
test('invalid requests fail and a fresh service has no history',async()=>{
 const s=createReviews();assert.throws(()=>s.start('missing','a'),/not found/)
 const r=await s.start(claims[0].id,'start')
 await assert.rejects(s.review(r.id,{...decision,reason:''},'a'))
 const fresh=createReviews();assert.equal(fresh.list().length,0)
