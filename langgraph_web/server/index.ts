...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (path.startsWith('/api')) {
      if (req.method === 'GET') {
        if (path === '/api/claims') return send(200, claims)
        if (path === '/api/reviews') return send(200, service.list())
        if (path === '/api/health') return send(200, { ok: true, storage: 'MemorySaver' })
        const m = path.match(/^\/api\/reviews\/([a-f0-9-]+)(\/history)?$/)
        if (m) return send(200, m[2] ? await service.history(m[1]) : service.get(m[1]))
      }
      ...
      if (req.method === 'POST') {
        ...
        for await (const chunk of req) {
          body += chunk
          if (Buffer.byteLength(body) > 16000) throw new HttpError(413, 'Request too large')
        }
        let input: unknown
        try {
        ...
        if (m) {
          ...
          return send(200, await service.review(m[1], p.decision, p.requestId))
        }
      }
    ...
    if (!file.startsWith(dist + '/')) throw new HttpError(404, 'Not found')
    try {
      const data = await readFile(file)
      res.writeHead(200, { 'Content-Type': mime[extname(file)] ?? 'application/octet-stream' })
      res.end(data)
    } catch {
      throw new HttpError(404, 'Build the UI with npm run build, or open the Vite URL on port 4317')
    }
  } catch (e) {
    send(e instanceof HttpError ? e.status : e instanceof ZodError ? 400 : 500, {
      error:
