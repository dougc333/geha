...
def run():
    ...
    for req in REQUESTS:
        ...
        if pa_req:
            run.log(
                tx, "REQUIREMENT", "PA_REQUIRED", f"{cpt} requires prior authorization"
            )
        elif mn_req:
            run.log(
                tx,
                "REQUIREMENT",
                "MN_REVIEW",
                f"{cpt} requires medical-necessity review",
            )
        else:
        ...
        if pa_req and score < 0.5:
            run.log(
                tx, "CLINICAL_REVIEW", "PENDING", "Flagged for manual medical review"
            )
        ...
        if score >= 0.7:
            ...
            run.log(
                tx, "CLINICAL_REVIEW", "APPROVED", f"Meets criteria; auth {auth_num}"
            )
    ...
    events = [
        {"tx_id": e.tx_id, "stage": e.stage, "status": e.status, "note": e.note}
        for e in run.events
    ]
    ...
    for a in run.auths:
        print(
            f"[{a.tx_id}] member {a.member}  {a.cpt:6}  {a.decision:14} {a.auth_number}"
        )
