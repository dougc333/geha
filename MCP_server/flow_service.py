...
FLOWS = {
    '01': Flow('01_membership_benefits', 'simulate_membership.py', 'members.json', 'membership_events.json'),
    '02': Flow('02_provider_operations', 'simulate_providers.py', 'providers.json', 'provider_events.json'),
    '03': Flow('03_utilization_management', 'simulate_utilization.py', 'authorizations.json', 'um_events.json'),
    '04': Flow('04_premium_billing', 'simulate_premium_billing.py', 'premium_ledger.json', 'billing_events.json', ('01',)),
    '05': Flow('05_appeals_disputes', 'simulate_appeals.py', 'appeals.json', 'appeals_events.json'),
    '06': Flow('06_payment_integrity', 'simulate_payment_integrity.py', 'screening_log.json', 'integrity_events.json'),
    '07': Flow('07_care_case_management', 'simulate_care_management.py', 'care_cases.json', 'care_events.json'),
    '08': Flow('08_member_services', 'simulate_member_services.py', 'inquiries.json', 'service_events.json', ('01', '03')),
    '09': Flow('09_compliance', 'simulate_compliance.py', 'compliance_register.json', 'compliance_events.json', tuple(f'{i:02}' for i in range(1, 9))),
}
...
class FlowService:
    def __init__(self, root: Path, runs: Path, claims: Path, timeout=30, max_runs=100):
        self.root, self.runs, self.claims = root.resolve(), runs.resolve(), claims.resolve()
    ...
    def list_flows(self):
        return {'simulation_only': True, 'flows': [
            {'flow_id': key, 'name': f.folder, 'dependencies': list(f.dependencies),
    ...
    async def execute(self, requested):
        ...
        async with self.slots:
            ...
            if sum(p.is_dir() for p in self.runs.glob('run_*')) >= self.max_runs:
                raise RuntimeError('Run quota reached; archive old run folders before retrying')
            run_id = 'run_' + uuid.uuid4().hex
            work = self.runs / run_id
            work.mkdir(mode=0o700)
            manifest = {'run_id': run_id, 'status': 'running', 'started_at': now(),
                        'requested': requested, 'execution_order': plan,
                        'completed_flows': [], 'warnings': [], 'script_sha256': {},
            ...
            try:
                ...
                for key in plan:
                    ...
                    if original.is_symlink() or not original.resolve().is_relative_to(self.root):
                ...
                if any(key in plan for key in ('08', '09')):
                    if self.claims.exists():
                        ...
                        if not isinstance(claims, list) or any(not isinstance(r, dict) or not isinstance(r.get('claim_id'), str) or not isinstance(r.get('final_status'), str) for r in claims):
                            raise ValueError('Claims audit input must be a list of claim/status records')
                        claims_dest.parent.mkdir()
                        write_json(claims_dest, claims)
                        manifest['claims_snapshot'] = {
                            'source': str(self.claims), 'records': len(claims),
                            'sha256': hashlib.sha256(claims_dest.read_bytes()).hexdigest(),
                            'note': 'Existing audit data snapshot; claims adjudication was not rerun.'}
                    else:
                        manifest['warnings'].append('Claims audit input is missing; lookups may return NOT_FOUND and compliance evidence will be incomplete.')
                for key in plan:
                    await self._run_script(work, key, claims_dest)
                    f = self.flow(key)
                    folder = work / f.folder
                    payload = read_json(folder / 'mcp_response.json')
                    if (not isinstance(payload, dict) or payload.get('flow') != f.folder
                ...
                write_json(work / 'run.json', manifest)
                return {'run_id': run_id, 'execution_status': 'completed', 'simulation_only': True,
                        'execution_order': plan, 'warnings': manifest['warnings'],
                        'results': {k: read_json(work / FLOWS[k].folder / 'mcp_response.json') for k in requested}}
            except BaseException as exc:
                manifest['status'] = 'cancelled' if isinstance(exc, asyncio.CancelledError) else 'failed'
                manifest['finished_at'] = now()
                message = 'Simulator exceeded its execution timeout' if isinstance(exc, TimeoutError) else str(exc)
    ...
    async def _run_script(self, work, key, claims_dest):
        ...
        proc = await asyncio.create_subprocess_exec(
            sys.executable, '-B', str(folder / f.script), cwd=folder, env=env,
        ...
        async def collect():
            stdout, stderr = await asyncio.gather(drain(proc.stdout), drain(proc.stderr))
            await proc.wait()
        ...
        try:
            ...
            if proc.returncode != 0:
                raise RuntimeError(f'Flow {key} exited with status {proc.returncode}; see isolated stderr.log')
    ...
    def records(self, flow_id, run_id, kind, offset=0, limit=50):
        ...
        return {'run_id': run_id, 'flow_id': flow_id, 'total': len(data),
                'offset': offset, 'items': data[offset:offset+limit],
