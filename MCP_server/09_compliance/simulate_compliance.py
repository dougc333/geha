...
def run():
    ...
    if file_exists(prov) and file_exists(auth):
        run.log(
            "ACCREDITATION", "STANDARD", "OK", "Credentialing & UM evidence present"
        )
        run.register.append(
            ComplianceRecord(
                "Accreditation_NCQA_URAC",
                "COMPLIANT",
        ...
        run.log(
            "ACCREDITATION", "STANDARD", "FLAG", "Missing credentialing/UM evidence"
        )
        run.register.append(
            ComplianceRecord(
                "Accreditation_NCQA_URAC",
    ...
    if privacy_ok:
        run.log(
            "HIPAA_PRIVACY", "PHI", "OK", "PHI access monitored; no unresolved breach"
        )
        run.register.append(
            ComplianceRecord(
        ...
        run.register.append(
            ComplianceRecord(
                "HIPAA_Privacy_Security", "BREACH_REPORTED", "PHI breach reported"
            )
        )
    ...
    if file_exists(appeals):
        run.log(
            ...
            "AUDIT",
            "OK",
            "Appeal/grievance compliance evidence present",
        )
        run.register.append(
            ComplianceRecord(
        ...
        run.register.append(
            ComplianceRecord(
                "Audit_Internal_Controls", "REMEDIATION", "Missing appeal evidence"
            )
        )

    # Persist
    register = [
        {"area": r.area, "status": r.status, "findings": r.findings}
        for r in run.register
    ]
    (HERE / "compliance_register.json").write_text(json.dumps(register, indent=2))
    events = [
        {"area": e.area, "stage": e.stage, "status": e.status, "note": e.note}
        for e in run.events
    ]
    ...
    response = {
        ...
        "warnings": [
            "Evidence checks primarily test file existence.",
            "HIPAA check is an always-pass placeholder.",
        ],
