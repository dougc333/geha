...
MEMBERS = (
    json.loads((_ROOT / "01_membership_benefits" / "members.json").read_text())
    if (_ROOT / "01_membership_benefits" / "members.json").exists()
    else []
)
AUTHS = (
    json.loads(
        (_ROOT / "03_utilization_management" / "authorizations.json").read_text()
    )
    if (_ROOT / "03_utilization_management" / "authorizations.json").exists()
    else []
)
...
def run():
    ...
    for itype, mid, ref, verified in INQUIRIES:
        ...
        if not verified:
            run.log(
                iid, "INTAKE", "FAIL", "Identity not verified (member ID/DOB mismatch)"
            )
            run.inquiries.append(
                InquiryRecord(
                    iid,
                    itype,
                    mid,
        ...
        if itype == "CLAIM_STATUS":
            ...
            run.inquiries.append(
                InquiryRecord(
                    ...
                    ref,
                    True,
                    "Premium Billing (Flow 4)",
                    "RESOLVED",
                    "Premium balance provided",
                )
            )
            ...
            run.inquiries.append(
                InquiryRecord(
                    iid, itype, mid, ref, True, "", "ESCALATED", "Routed to specialist"
                )
            )
    ...
    response = {
        ...
        "warnings": [
            "Some responses are placeholders.",
            "The current code can mark a NOT_FOUND lookup as RESOLVED.",
        ],
    ...
    for i in run.inquiries:
        print(
            f"[{i.inquiry_id}] {i.itype:14} member={i.member} ref={i.ref:12} "
