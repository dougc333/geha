...
FACILITIES = [
    ...
    "Capital Physician Group",
    "Fairfax Family Practice",
    "Apex Orthopedic Associates",
    "Midwest Cardiology Clinic",
]
...
def run():
    ...
    for p in PROVIDERS:
        ...
        if not p["board"]:
            run.log(
                npi, "CREDENTIALING", "FLAG", "Board certification missing (non-fatal)"
            )
        ...
        if p["name"].startswith("Dr. Y."):
            ...
            run.log(
                npi, "CONTRACTING", "OK", "Credentialed but practicing out-of-network"
            )
        else:
            ...
            run.log(
                ...
                "CONTRACTING",
                "OK",
                f"Participating contract; allowed = 85% of billed",
            )
        ...
        roster.append(
            {
                ...
                "status": r.status,
                "network": r.network,
                "allowed_factor": r.allowed_factor,
                "facility": r.facility,
            }
        )
    (HERE / "providers.json").write_text(json.dumps(roster, indent=2))
    events = [
        {"npi": e.npi, "stage": e.stage, "status": e.status, "note": e.note}
        for e in run.events
    ]
    ...
    response = {
        ...
        "summary": {
            ...
            "in_network": sum(
                r["status"] == "CREDENTIALED" and r["network"] == "IN_NETWORK"
                for r in roster
            ),
            "out_of_network": sum(
                r["status"] == "CREDENTIALED" and r["network"] == "OUT_OF_NETWORK"
                for r in roster
            ),
        },
    ...
    for r in run.providers.values():
        print(
            f"[{r.npi}] {r.name:18} {r.status:14} {r.network:16} "
