...
class RunnerTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.claims.write_text(json.dumps([{'claim_id':'CLM-100005', 'final_status':'PAID'}]))
        self.service = FlowService(self.source, self.base / 'runs', self.claims, timeout=5)
    ...
    async def test_all_flows_and_provenance(self):
        ...
        self.assertEqual(result['results']['09']['summary']['evidence_files_present'], 9)
        self.assertFalse(result['results']['09']['compliance_verified'])
        records = self.service.records('08', rid, 'records')['items']
        self.assertTrue(any(r['resolution'] == 'Claim CLM-100005 status: PAID' for r in records))
    ...
    async def test_prerequisites_and_parallel_isolation(self):
        a, b = await asyncio.gather(self.service.execute(['04']), self.service.execute(['04']))
    ...
    async def test_missing_claims_warning(self):
        ...
        self.assertEqual(result['results']['09']['summary']['evidence_files_missing'], 1)
    ...
    async def test_invalid_payload(self):
        self.replace_script('from pathlib import Path; Path("mcp_response.json").write_text("{}")')
    ...
    async def test_stdio_tools(self):
        with tempfile.TemporaryDirectory() as tmp:
            params = StdioServerParameters(command=sys.executable, args=[str(ROOT / 'server.py')],
                env={'GEHA_RUNS_DIR': tmp, 'GEHA_CLAIMS_PATH': str(Path(tmp) / 'missing.json')})
            async with stdio_client(params) as (read, write):
                async with ClientSession(read, write) as session:
                    await session.initialize()
                    tools = (await session.list_tools()).tools
                    self.assertEqual({t.name for t in tools}, {'list_flows','run_flow','run_all_flows',
                        'get_flow_results','get_flow_records','get_flow_events','get_run_status'})
                    ...
                    invalid = await session.call_tool('run_flow', {'flow_id':'../../etc'})
                    ...
                    rid = payload['run_id']
                    for key in FLOWS:
                        read_result = await session.call_tool('get_flow_results', {'flow_id': key, 'run_id': rid})
                        self.assertFalse(read_result.isError)
                        self.assertEqual(read_result.structuredContent['run_id'], rid)
                    events = await session.call_tool('get_flow_events', {'flow_id':'01','run_id':rid,'limit':2})
                    self.assertEqual(len(events.structuredContent['items']), 2)
                    bad = await session.call_tool('get_flow_results', {'flow_id':'01','run_id':'../bad'})
