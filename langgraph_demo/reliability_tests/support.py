...
class Fixture(unittest.TestCase):
    def setUp(self):
        ...
        self.sources = {
            folder/'claims.json': [{'claim_id':'CLM-1','member_id':'M1','plan':'Synthetic'}],
            folder/'audit_trails.json': [{'claim_id':'CLM-1','final_status':'PENDING_REVIEW',
                'stages':[{'status':'PENDING','note':'Prior auth review'}]}],
            refs/'public_reference.json': [],
        }
        for p, value in self.sources.items(): p.write_text(json.dumps(value))
        self.originals = {p:p.read_bytes() for p in self.sources}
        self.addCleanup(self.check_sources)

    def check_sources(self):
        for p, value in self.originals.items(): self.assertEqual(p.read_bytes(),value)
    ...
    def launch(self, mode, action='approve'):
        ...
        proc = ctx.Process(target=worker,args=(child,self.base,self.db,mode,action))
    ...
    def inspect(self):
        ...
        result = self.receive(pipe,'result'); proc.join(5)
