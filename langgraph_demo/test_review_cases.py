...
class ReviewCasesTests(unittest.TestCase):
    def setUp(self):
        ...
        claims = [{'claim_id': cid, 'member_id': f'M{i}', 'plan': 'Synthetic',
                   'status': self.statuses[cid]} for i, cid in enumerate(self.ids)]
        trails = [{'claim_id': cid, 'final_status': self.statuses[cid],
                   'stages': [{'status': 'PENDING', 'note': 'Requires prior auth review'}]
                   if self.statuses[cid] == 'PENDING_REVIEW' else []} for cid in self.ids]
    ...
    def test_two_new_reviews_for_same_claim_have_independent_histories(self):
        ...
        with workflow(self.db, self.base) as g:
            for tid in [first, second]:
                g.invoke({'claim_id': self.ids[0], 'actor': 'demo_operator'}, self.config(tid))
    ...
    def test_all_20_saved_reviews_survive_reopen_and_keep_history(self):
        ...
        with workflow(self.db, self.base) as g:
            before = {cid: list(g.get_state_history(self.config(self.prepared_id(cid)))) for cid in self.ids}
        with workflow(self.db, self.base) as g:
            for cid in self.ids:
                with self.subTest(claim=cid):
                    ...
                    self.assertEqual([s.config for s in after], [s.config for s in before[cid]])

    def test_prepare_is_idempotent_and_preserves_completed_and_other_threads(self):
        self.assertEqual(prepare(self.db, self.base), 20)
        other = 'existing-manual-review'
        with workflow(self.db, self.base) as g:
            g.invoke({'claim_id': self.ids[0], 'actor': 'demo_operator'}, self.config(other))
    ...
    def test_reopened_reviews_can_approve_or_reject_all_20_without_claim_mutation(self):
        ...
        for i, cid in enumerate(self.ids):
            with self.subTest(claim=cid), workflow(self.db, self.base) as g:
                ...
                result = resume(g, tid, action, 'Checked evidence', edited_text='Reviewed text')
                self.assertEqual(result['review']['action'], action)
                self.assertEqual(result['facts']['status'], self.statuses[cid])
                self.assertEqual(result['answer'], 'Reviewed text' if action == 'approve'
    ...
    def test_expanded_operator_access_does_not_expand_member_or_outsider_access(self):
        with workflow(self.db, self.base) as g:
            for actor, cid in [('member', self.ids[1]), ('outsider', self.ids[0]),
    ...
    def test_streamlit_start_refresh_and_reopen_use_automatic_ids(self):
        ...
        prepare(self.db, self.base)
        with patch.dict(os.environ, {'GEHA_DEMO_BASE': str(self.base), 'GEHA_DEMO_DB': str(self.db)}):
            ...
            self.assertEqual(len(next(x for x in a.selectbox if x.label == 'Reopen a case').options), 21)
            ...
            with workflow(self.db, self.base) as g:
                self.assertEqual(g.get_state(self.config(tid)).values['claim_id'], self.ids[-1])
            next(x for x in b.text_input if x.label == 'Reason (required)').set_value('Verified')
