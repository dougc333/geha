...
def prepare(db, base):
    ...
    with workflow(db,base) as g:
        for claim in sorted(claims):
            if claim not in trails: raise ValueError(f'Missing audit trail for {claim}')
            tid=str(uuid.uuid5(uuid.NAMESPACE_URL,'geha-demo/prepared-review/'+claim))
            cfg={'configurable':{'thread_id':tid}}
            if g.get_state(cfg).values: continue
...
if __name__=='__main__':
    print('Prepared new reviews:',prepare(ROOT/'data/checkpoints.sqlite',ROOT.parent))
