...
def inherit_continuation_header(
    ...
    *,
    ...
    A numeric DataFrame header alone is not evidence of a continuation. Require
    adjacent pages, the same non-generic section heading, matching column count,
    and recognizable named columns on the preceding table. No data row is dropped.
    """
    if previous_frame is None or frame.empty or previous_frame.empty:
        return frame, False
    if (
        ...
        return frame, False
    if [str(column).strip() for column in frame.columns] != [
        str(index) for index in range(frame.shape[1])
    ]:
        return frame, False
