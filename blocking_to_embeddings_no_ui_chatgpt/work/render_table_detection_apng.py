...
def font(size, bold=False):
    return ImageFont.truetype(
        str(FONT_DIR / ("Arial Bold.ttf" if bold else "Arial.ttf")), size
    )
...
def detect(page):
    ...
    vertical = [
        ...
        for e in pdfplumber.utils.rect_to_edges(r)
        if e["orientation"] == "v"
    ]
    horizontal = [
        e
    ...
    return sorted(
        [t for t in tables if len(t.columns) >= 2 and len(t.rows) >= 2],
...
def frame(record, page_image, crop, phase, index, total, source):
    ...
    d.text(
        (W - 24, 23),
        f"TABLE {index + 1} OF {total}  /  PAGE {record['page']}",
        font=font(15),
        fill=MUTED,
        anchor="ra",
    )
    ...
    d.rounded_rectangle(
        (670, 268, 1400, 578), radius=10, fill="#f6f8fb", outline="#d9dee7"
    )
    if phase:
    ...
    d.text(
        (670, 603),
        f"{record['rows_including_header']} rows (including header)  ×  {record['columns']} columns",
        font=font(22, True),
        fill=INK,
    )
    ...
    d.text(
        (670, 670),
        "  ".join(f"{v:.2f}" for v in record["bbox_points"]),
        font=font(18),
        fill=INK,
    )
    ...
    d.text(
        (670, 839),
        "Recorded detections • embedded PDF text • no OCR",
        font=font(15),
        fill=MUTED,
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT.parent
        / "downloads/coverage-policies/geha-coverage-policy-bendamustine.pdf",
    )
    ...
    with pdfplumber.open(source) as pdf:
        for page_no, page in enumerate(pdf.pages, 1):
            ...
            for table_no, table in enumerate(tables, 1):
                ...
                crop_box = (
                    max(0, math.floor((bbox[0] - 4) * sx)),
                    max(0, math.floor((bbox[1] - 4) * sy)),
                    min(page_image.width, math.ceil((bbox[2] + 4) * sx)),
                    min(page_image.height, math.ceil((bbox[3] + 4) * sy)),
                )
                crop = page_image.crop(crop_box)
                path = crop_dir / f"page-{page_no:02d}-table-{table_no:02d}.png"
                crop.save(path, dpi=(216, 216))
                rows = [
                    [re.sub(r"\s+", " ", c or "").strip() for c in r]
                    for r in table.extract()
                ]
                record = {
                    "page": page_no,
                    "table_on_page": table_no,
                    "bbox_points": bbox,
                    "page_size_points": [page.width, page.height],
                    "cells_points": table.cells,
                    "rows_including_header": len(rows),
                    "columns": len(table.columns),
                    "rows": rows,
                    "screenshot": str(path.relative_to(output)),
                    "crop_pixels": crop_box,
                    "size_pixels": list(crop.size),
                    "screenshot_sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
                }
                records.append(record)
                previews.append((page_image.copy(), crop))
    renderer.close()
    if not records:
        raise SystemExit(
            "No supported bordered tables detected. Borderless/scanned tables need another detector."
        )
    manifest = {
    ...
    frames = [
        frame(r, *previews[i], phase, i, len(records), source)
    ...
    frames[0].save(
        ...
        duration=[1400, 1700, 2200] * len(records),
        loop=0,
        disposal=1,
        blend=0,
    )
        ...
        json.dumps(
            {
                ...
                "tables": len(records),
                "frames": len(frames),
                "manifest": str(crop_dir / "manifest.json"),
            },
            indent=2,
        )
    )
