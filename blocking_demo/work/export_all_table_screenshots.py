...
def export(source, destination):
    ...
    with pdfplumber.open(source) as pdf, pdfium.PdfDocument(source) as renderer:
        for number, page in enumerate(pdf.pages, 1):
            ...
            for index, table in enumerate(
                sorted(tables, key=lambda t: (t.bbox[1], t.bbox[0])), 1
            ):
                x0, y0, x1, y1 = table.bbox
                sx, sy = image.width / page.width, image.height / page.height
                box = (
                    max(0, math.floor((x0 - 4) * sx)),
                    max(0, math.floor((y0 - 4) * sy)),
                    min(image.width, math.ceil((x1 + 4) * sx)),
                    min(image.height, math.ceil((y1 + 4) * sy)),
                )
                crop = image.crop(box)
                ...
                records.append(
                    {
                        ...
                        "table_on_page": index,
                        "method": method,
                        "bbox_points": list(table.bbox),
                        "page_size_points": [page.width, page.height],
                        "rows": rows,
                        "text_available": any(any(c for c in row) for row in rows),
    ...
    return {
        ...
        "pages_without_text": [p["page"] for p in pages if p["review_needed"]],
    }, records
...
def main():
    ...
    sources = sorted(
        p for p in args.source_dir.glob("*.pdf") if not p.stem.endswith("_openai")
    )
    summary, previews = [], []
    for source in sources:
        try:
            ...
            previews.extend(
                (Path(entry["folder"]) / r["screenshot"], source.stem, r)
                for r in records
            )
    ...
    report = {
        ...
        "tables": sum(s["tables"] for s in summary),
        "results": summary,
    }
    ...
    for offset in range(0, len(previews), 12):
        ...
        for j, (path, name, record) in enumerate(group):
            ...
            draw.text(
                (x + 8, y + 22),
                f"Page {record['page']}, table {record['table_on_page']}: {record['rows_including_header']} rows x {record['columns']} cols",
                fill="black",
            )
