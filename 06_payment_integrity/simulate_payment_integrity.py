...
CLAIMS_TO_SCREEN = [
    {"claim_id": "CLM-100000", "cpt": "99204", "dx": "H25.13", "charge": 310.0, "flag": "none"},
    {"claim_id": "CLM-100002", "cpt": "90670", "dx": "Z23",    "charge": 250.0, "flag": "duplicate"},
    {"claim_id": "CLM-100004", "cpt": "95782", "dx": "G47.33", "charge": 1450.0, "flag": "none"},
    {"claim_id": "CLM-100007", "cpt": "99215", "dx": "I10",    "charge": 265.0, "flag": "upcode"},
    {"claim_id": "CLM-100012", "cpt": "99214", "dx": "J45.909","charge": 230.0, "flag": "unbundle"},
    {"claim_id": "CLM-100014", "cpt": "74178", "dx": "J18.9",  "charge": 950.0, "flag": "illogical_pair"},
    {"claim_id": "CLM-100016", "cpt": "90670", "dx": "Z23",    "charge": 250.0, "flag": "duplicate"},
    {"claim_id": "CLM-100017", "cpt": "99214", "dx": "M54.5",  "charge": 210.0, "flag": "none"},
    {"claim_id": "CLM-100019", "cpt": "99213", "dx": "N39.0",  "charge": 158.0, "flag": "none"},
]
...
def run():
    ...
    for c in CLAIMS_TO_SCREEN:
        ...
        if c["flag"] == "duplicate":
            flags.append("DUPLICATE_BILLING"); risk += 0.9
        if c["flag"] == "upcode":
            flags.append("UPCODING"); risk += 0.7
        if c["flag"] == "unbundle":
            flags.append("UNBUNDLING"); risk += 0.6
        if (c["cpt"], c["dx"]) in ILLOGICAL:
            flags.append("ILLOGICAL_DX_PROC_PAIR"); risk += 0.8
        if c["charge"] > 1200:
            flags.append("HIGH_CHARGE"); risk += 0.3

        if not flags:
            risk = 0.1
            run.log(cid, "PREPAY_SCREEN", "CLEAN", "No red flags; auto-pass")
            run.screens.append(ScreenRecord(cid, c["cpt"], c["dx"], c["charge"], [], 0.1, "CLEAN"))
            continue

        run.log(cid, "PREPAY_SCREEN", "HELD", f"Risk {risk:.0%}; held for review ({', '.join(flags)})")

        # Stage 2 — Investigation / classification
        if risk >= 0.8:
            cls = "FRAUD_REFERRED"
            run.log(cid, "INVESTIGATION", cls, "SIU review; suspected fraud -> FCA referral")
        elif risk >= 0.5:
            cls = "OVERPAYMENT_RECOVERED" if c["charge"] > 600 else "ADJUSTED"
            if cls == "ADJUSTED":
                run.log(cid, "CORRECTION", "ADJUSTED", "Charges adjusted pre-payment")
            else:
                run.log(cid, "CORRECTION", "RECOVERED", "Post-payment overpayment recovery initiated")
        else:
            cls = "ERROR_CORRECTED"
            run.log(cid, "CORRECTION", "ERROR_CORRECTED", "Billing error corrected")

        run.screens.append(ScreenRecord(cid, c["cpt"], c["dx"], c["charge"], flags, risk, cls))

    # Persist
    screens = [{"claim_id": s.claim_id, "cpt": s.cpt, "dx": s.dx, "charge": s.charge,
                "flags": s.flags, "risk": round(s.risk, 2), "decision": s.decision}
               for s in run.screens]
    (HERE / "screening_log.json").write_text(json.dumps(screens, indent=2))
    events = [{"claim_id": e.claim_id, "stage": e.stage, "status": e.status, "note": e.note}
              for e in run.events]
    ...
    for s in run.screens:
        ...
        print(f"[{s.claim_id}] {s.cpt:6} {s.dx:10} ${s.charge:7.2f} risk={s.risk:.2f} "
              f"{s.decision:20} ({fl})")
