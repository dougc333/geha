...
const claimSchema = z.object({
  ...
  member: z.string(),
  plan: z.string(),
  status: z.enum(['PENDING_REVIEW', 'PAID', 'DENIED', 'PARTIAL']),
  reason: z.string(),
...
export const decisionSchema = z
  ...
  .strict()
export const claims: Claim[] = z
  .array(claimSchema)
  .parse(JSON.parse(readFileSync(new URL('./data/claims.json', import.meta.url), 'utf8')))
...
export function createReviews() {
  ...
  const graph = new StateGraph(State)
    .addNode('explain', (s) => {
      ...
      return {
        draft: `SIMULATION ONLY\n\nClaim ${c.id} has recorded status ${c.status}.\n\n${c.reason}\n\nNext step: have the demo reviewer verify this explanation. Approval releases this text only; it does not change the insurance claim.`,
        answer: '',
      }
    })
    .addNode('human_review', (s) => {
      ...
      return {
        decision: d,
        answer: d.action === 'approve' ? d.text : 'Explanation rejected. No response released.',
      }
    })
    .addEdge(START, 'explain')
    .addEdge('explain', 'human_review')
  ...
  function get(id: string) {
    ...
    if (!r) throw new HttpError(404, 'Review not found; the server may have restarted')
    return r
  }
  return {
    ...
    start(claimId: string, requestId: string): Promise<Review> {
      ...
      if (prior) {
        if (prior.claimId !== claimId)
          throw new HttpError(409, 'Request ID reused with different claim')
        return prior.promise
      }
      if (!claims.some((c) => c.id === claimId)) throw new HttpError(404, 'Claim not found')
      if (starts.size >= 200)
        throw new HttpError(429, 'Demo session limit reached. Restart the backend to clear memory.')
      const promise = (async () => {
        const id = randomUUID()
        const s = await graph.invoke({ claimId }, cfg(id))
        const r: Review = {
          id,
          claimId,
      ...
      starts.set(requestId, { claimId, promise })
      return promise
    },
    async review(id: string, raw: unknown, requestId: string) {
      ...
      if (old) {
        if (old.payload !== payload)
          throw new HttpError(409, 'Request ID reused with different decision')
        return old.result
      }
      const current = get(id)
      if (busy.has(id) || current.status !== 'waiting')
        throw new HttpError(409, 'Case is being processed or already reviewed')
      busy.add(id)
      try {
        ...
        records.set(id, r)
        submitted.set(key, { payload, result: r })
        return r
      } finally {
        busy.delete(id)
      }
    ...
    async history(id: string) {
      get(id)
      const rows = []
      for await (const s of graph.getStateHistory(cfg(id)))
        rows.push({ createdAt: s.createdAt, next: s.next, state: s.values })
      return rows.reverse()
    },
  }
}
