...
FIELDS = ["diagnosis", "cpt", "pos", "dos", "charge", "npi", "provider", "facility", "member_id", "patient_name"]
r = vdb.VectorResolver()

per_field = {f: {"correct": 0, "present": 0, "wrong": 0, "unrecovered": 0, "filled_but_missing": 0} for f in FIELDS}
for c in gt:
...
print(f"{'field':14} {'correct':>8} {'unrecover':>10} {'wrong':>6} {'recover_acc':>12} {'miss_ok':>8}")
for f in FIELDS:
    d = per_field[f]
    ...
    print(f"{f:14} {d['correct']:>4}/{d['present']:<3} {d['unrecovered']:>10} "
          f"{d['wrong']:>6} {rec_acc:>11.0%} {d['filled_but_missing']:>8}")
# aggregate
tot_c=sum(d['correct'] for d in per_field.values()); tot_p=sum(d['present'] for d in per_field.values())
tot_w=sum(d['wrong'] for d in per_field.values()); tot_u=sum(d['unrecovered'] for d in per_field.values())
print(f"\nTOTAL  correct={tot_c}/{tot_p}  unrecovered={tot_u} (->missing)  truly_wrong={tot_w}")
print(f"  overall recovery accuracy: {tot_c/tot_p:.0%}; +unrecovered-as-missing keeps {tot_u} fields honest")
...
for c in gt[:4]:
    ...
    print(c['claim_id'], "diag_truth=", repr(c['diagnosis']), "cpt_truth=", repr(c['cpt']),
          "charge_truth=", repr(c['charge']))
    print("   tokens:", toks)
