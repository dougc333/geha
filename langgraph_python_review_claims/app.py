...
st.warning(
    "Demo operator console: all saved cases are visible locally. Personas are not secure authentication. Synthetic data only."
)

try:
    claims, trails, refs, example = load_inputs(BASE)
except (OSError, ValueError, KeyError) as exc:
    st.error(f"Cannot load demo inputs: {exc}")
    st.stop()
...
with st.sidebar:
    ...
    if not cases:
        st.info("Start a case to review its explanation.")
        st.stop()
    if st.session_state.get("case_select") not in cases:
        st.session_state["case_select"] = cases[0]
    tid = st.selectbox(
        "Reopen a case", cases, key="case_select", format_func=lambda t: f"Case {t[:8]}"
    )
...
with left:
    ...
    if not s.get("authorized"):
        st.error(s.get("answer", "Access denied."))
        ...
        with st.form(f"review_{tid}"):
            ...
            action = st.radio(
                "Reviewer decision", ["approve", "reject"], horizontal=True
            )
            reason = st.text_input("Reason (required)")
            submitted = st.form_submit_button("Submit review", type="primary")
        if submitted:
            if not reason.strip():
                st.error("Enter a review reason.")
            elif action == "approve" and not text.strip():
                st.error("An approved explanation cannot be empty.")
            else:
                try:
                    request_key = f"review_request_id_{tid}"
        ...
        st.success(
            ...
            else "Explanation rejected"
        )
        st.write(s.get("answer", ""))
        st.caption("Reviewer reason: " + s["review"]["reason"])
    else:
        st.info("No pending review. Inspect the checkpoint history below.")
with right:
    st.subheader("Recorded facts")
    if s.get("facts"):
        st.json(s["facts"])
    else:
        st.caption("No claim details were retrieved.")
    st.caption(
        "General guidance is not a plan-specific coverage determination. No LLM is used."
    )
