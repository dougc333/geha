...
class DemoTests(unittest.TestCase):
    def setUp(self):
        ...
        (folder / "audit_trails.json").write_text(
            json.dumps(
                [
                    {
                        "claim_id": "CLM-1",
                        ...
                        "stages": [
                            {"status": "PENDING", "note": "Requires prior auth review"}
                        ],
                    }
                ]
            )
        ...
        (folder / "public_reference.json").write_text(
            json.dumps(
                [
                    {
                        "text": "Ask the provider for assistance.",
                        "metadata": {
                            "visibility": "public",
    ...
    def test_reject(self):
        with workflow(self.db, self.base) as g:
            self.start(g)
            result = resume(
                g, "test", "reject", "Needs better evidence", request_id="reject-1"
            )
            self.assertEqual(
                result["answer"], "Explanation rejected; no response released."
            )
            self.assertEqual(result["review"]["request_id"], "reject-1")
            self.assertEqual(result["review"]["actor_id"], "demo-reviewer")
            self.assertEqual(result["audit_event"]["event_type"], "review.reject")
            self.assertEqual(result["audit_event"]["request_id"], "reject-1")
            self.assertEqual(
                result["audit_event"]["payload_hash"], result["review"]["payload_hash"]
            )
    ...
    def test_edit_and_review_permissions(self):
        with workflow(self.db, self.base) as g:
            self.start(g)
            with self.assertRaises(ValueError):
                resume(g, "test", "approve", "Reason", reviewer="member")
            with self.assertRaises(ValueError):
                resume(g, "test", "approve", " ")
            result = resume(
                g,
                "test",
                "approve",
                "Edited wording",
                edited_text="Reviewed explanation",
            )
