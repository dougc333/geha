...
def run_evals(retriever, generate=False, base_url="", model="", api_key=""):
    ...
    for case in json.loads((ROOT / "evals/cases.json").read_text()):
        ...
        expected = [
            ...
            if c["metadata"]["document_id"] == case["document"]
            and c["metadata"]["page"] == case["page"]
        ]
        evidence = " ".join(c["text"] for c in expected).lower()
        hit = (
            bool(expected)
            and all(term.lower() in evidence for term in case["required_terms"])
            if case["answerable"]
            else None
        )
        results.append(
            {
                ...
                "retrieval_pass": hit,
                "answer": state["answer"] if generate else None,
                "abstention_pass": state["abstained"] == (not case["answerable"])
                if generate
                else None,
                "citation_ids": [c["id"] for c in state["citations"]],
                "retrieved_ids": [c["id"] for c in state["chunks"]],
            }
        )
    return results
...
if __name__ == "__main__":
    ...
    rows = run_evals(
        ...
        os.getenv("MODEL_NAME", "llama3.2:3b"),
    ...
    print(
        f"Retrieval evidence checks: {sum(r['retrieval_pass'] for r in graded)}/{len(graded)}"
    )
    print(
        "Answer correctness requires review; retrieval success does not prove answer accuracy."
    )
    print(path)
